...
# reduce non-parallel data to same size
...
if args.n_references is not None:
    # If dataset has parallel references
    # parallel dataset for evaluation of set A to set B
    ...
    # parallel dataset for evaluation of set B to set A
...
if args.n_references is not None:
    # Load dataloaders for evaluation if parallel references
...
# Load checkpoints if from_pretrained is true
if args.from_pretrained is not None:
...
for epoch in range(start_epoch, args.epochs):
    ...
    for unsupervised_a, unsupervised_b in zip(mono_dl_a, mono_dl_b):
        ...
        # Why it never uses this condition??
    ...
    if epoch%args.save_steps==0:
        ...
        print(f"checkpoint saved at epoch {epoch} at location {args.save_base_folder}epoch_{epoch}/checkpoint.pth")
