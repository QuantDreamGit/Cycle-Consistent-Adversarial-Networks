...
class StarGANModel(nn.Module):
    ...
    def training_step(
        ...
        # Classify the transferred sentences to check if they actually belong to the target sentiment
        predicted_domains = torch.argmax(
            self.D(transferred_sentences, device=self.device)["domain_logits"], dim=1
        )

        # Ensure the predicted sentiment matches the intended target sentiment
        valid_transfers = predicted_domains == torch.tensor(target_styles, device=self.device)

        if valid_transfers.any():  # Only train on correctly transferred samples
            sentences_filtered = [sent for i, sent in enumerate(sentences) if valid_transfers[i]]
            transferred_sentences = [sent for i, sent in enumerate(transferred_sentences) if valid_transfers[i]]
            source_styles = [style for i, style in enumerate(source_styles) if valid_transfers[i]]
            target_styles = [style for i, style in enumerate(target_styles) if valid_transfers[i]]
            ...
            _, reconstructed_sentences, loss_cycle = self.G(transferred_sentences, source_styles, sentences_filtered, device=self.device)
        ...
        labels_real_sentences = torch.column_stack((torch.ones(len(sentences), device=self.device), torch.zeros(len(sentences), device=self.device)))
        ...
        if valid_transfers.any():  # Only train discriminator if there are valid fake sentences
            labels_fake_sentences = torch.column_stack((torch.zeros(len(transferred_sentences), device=self.device), torch.ones(len(transferred_sentences), device=self.device)))
