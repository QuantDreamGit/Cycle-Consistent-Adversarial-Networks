...
class Evaluator():
    ...
    def __compute_metric__(self, predictions, references, metric_name, direction=None):
        ...
        if metric_name in ['bleu', 'rouge', 'bertscore']:
            # Filter out predictions with empty references
            filtered_predictions = []
            filtered_references = []

            for pred, ref in zip(predictions, references):
                if ref:  # i.e., if ref is not an empty list
                    filtered_predictions.append(pred)
                    filtered_references.append(ref)

            if filtered_predictions:  # Perform metric computation only if there are valid references
                for pred, ref in zip(filtered_predictions, filtered_references):
                    if metric_name == 'bleu':
                        ...
                        for r in ref:
                            ...
                            tmp_rouge1.append(res['rouge1'])
                            tmp_rouge2.append(res['rouge2'])
                            tmp_rougeL.append(res['rougeL'])
                ...
                scores.extend([0])  # Append a default score if all references are empty
        else:
    ...
    def run_eval_ref_old(self, epoch, current_training_step, phase, parallel_dl_evalAB, parallel_dl_evalBA):
        print(f'Start {phase}...')
        self.cycleGAN.eval()  # set evaluation mode
    ...
    def run_eval_ref(self, epoch, current_training_step, phase, parallel_dl_evalAB, parallel_dl_evalBA):
        ...
        for batch in parallel_dl_evalAB:
            ...
            if self.args.lowercase_ref:
                references_b = [[ref.lower() for ref in refs] if refs else [] for refs in references_b]
            ...
            ref_B.extend(references_b)
            print("---------------------------------------------------------------------------------------------------")
            print("Original negative sentence: ", parallel_a[0])
            print("Transferred positive sentence: ", transferred[0])
            print("Reference positive sentence: ", references_b[0])
            print("---------------------------------------------------------------------------------------------------")

            parallel_a = [[s] for s in parallel_a]
            scores_AB_bleu_self.extend(self.__compute_metric__(transferred, parallel_a, 'bleu'))

            valid_refs = [refs for refs in references_b if any(refs)]
            if valid_refs:
                scores_AB_bleu_ref.extend(self.__compute_metric__(transferred, valid_refs, 'bleu'))
                scores_rouge_ref = np.array(self.__compute_metric__(transferred, valid_refs, 'rouge'))
                if scores_rouge_ref.ndim == 2:
                    scores_AB_r1_ref.extend(scores_rouge_ref[:, 0].tolist())
                    scores_AB_r2_ref.extend(scores_rouge_ref[:, 1].tolist())
                    scores_AB_rL_ref.extend(scores_rouge_ref[:, 2].tolist())
                else:
                    scores_AB_r1_ref.extend([0] * len(transferred))
                    scores_AB_r2_ref.extend([0] * len(transferred))
                    scores_AB_rL_ref.extend([0] * len(transferred))

                if self.args.bertscore:
                    scores_AB_bscore.extend(self.__compute_metric__(transferred, valid_refs, 'bertscore'))
            else:
                scores_AB_bleu_ref.extend([0] * len(transferred))
                scores_AB_r1_ref.extend([0] * len(transferred))
                scores_AB_r2_ref.extend([0] * len(transferred))
                scores_AB_rL_ref.extend([0] * len(transferred))
                if self.args.bertscore:
                    scores_AB_bscore.extend([0] * len(transferred))
        ...
        for batch in parallel_dl_evalBA:
            ...
            if self.args.lowercase_ref:
                references_a = [[ref.lower() for ref in refs] if refs else [] for refs in references_a]
            ...
            valid_refs = [refs for refs in references_a if any(refs)]
            if valid_refs:
                scores_BA_bleu_ref.extend(self.__compute_metric__(transferred, valid_refs, 'bleu'))
                scores_rouge_ref = np.array(self.__compute_metric__(transferred, valid_refs, 'rouge'))
                if scores_rouge_ref.ndim == 2:
                    scores_BA_r1_ref.extend(scores_rouge_ref[:, 0].tolist())
                    scores_BA_r2_ref.extend(scores_rouge_ref[:, 1].tolist())
                    ...
                    scores_BA_r1_ref.extend([0] * len(transferred))
                    scores_BA_r2_ref.extend([0] * len(transferred))
                    scores_BA_rL_ref.extend([0] * len(transferred))

                print("---------------------------------------------------------------------------------------------------")
                print("Original positive sentence: ", parallel_b[0])
                print("Transferred negative sentence: ", transferred[0])
                print("Reference negative sentence: ", references_a[0])
                print("---------------------------------------------------------------------------------------------------")
                print(f"Rouge 1: {scores_BA_r1_ref[-1]}")
                print(f"Rouge 2: {scores_BA_r2_ref[-1]}")
                print(f"Rouge L: {scores_BA_rL_ref[-1]}")

                if self.args.bertscore:
                    scores_BA_bscore.extend(self.__compute_metric__(transferred, valid_refs, 'bertscore'))
            else:
                scores_BA_bleu_ref.extend([0] * len(transferred))
                scores_BA_r1_ref.extend([0] * len(transferred))
                scores_BA_r2_ref.extend([0] * len(transferred))
                scores_BA_rL_ref.extend([0] * len(transferred))
                if self.args.bertscore:
                    scores_BA_bscore.extend([0] * len(transferred))
